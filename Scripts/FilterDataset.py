...
def filter_dataset(dataset):
    ...
    for index, move_probability in enumerate(dataset.move_probabilities):
        ...
        if calculate_entropy(move_probability) < 5:
...
if __name__ == '__main__':
    with open("../Datasets/quickstart_dataset_15x15_900_searches.p", 'rb') as train_file:
        dataset = pickle.load(train_file)
        filtered = filter_dataset(dataset)
    with open("../Datasets/quickstart_dataset_15x15_900_searches_filtered.p", 'wb') as file:
